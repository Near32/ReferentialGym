...
--dataset=SCS --dataset_length=1024 \
...
--epoch=1001 --graphtype=obverter --lr=0.0001 --max_sentence_length=10 \
--metric_active_factors_only=True --metric_batch_size=64 --dis_metric_epoch_period=201 --metric_epoch_period=10 --metric_resampling=True --mini_batch_size=64 \
--scs_nbr_latents=4 --scs_min_nbr_values_per_latent=5 \
--scs_max_nbr_values_per_latent=5 --scs_nbr_object_centric_samples=4 \
...
--nbr_eval_points=1000 --nbr_train_points=1000 \
...
--obverter_threshold_to_stop_message_generation=0.80 --obverter_use_residual_connections=False \
