...
def main():
  ...
  if 'Santoro2017-SoC' in agent_config['architecture']:
    ...
    agent_config['attention_model_hidden_size'] = 256
    agent_config['attention_speaker_soft_attention'] = args.attention_speaker_soft_attention
    agent_config['attention_listener_soft_attention'] = args.attention_listener_soft_attention
    agent_config['attention_st_gs_inv_tau0'] = args.attention_st_gs_inv_tau0
    agent_config['attention_visual_encoder_use_coord4'] = args.attention_visual_encoder_use_coord4
    agent_config['attention_full_focus_scheme'] = args.attention_full_focus_scheme
    ...
    agent_config['visual_decoder_nbr_steps'] = args.visual_decoder_nbr_steps
  ...
  if 'Attention' in args.agent_type and 'AttentionSpeaker' not in args.agent_type:
    ...
    save_path += f"-AttListSTGS{args.attention_st_gs_inv_tau0}/"
  ...
  if 'obverter' in args.graphtype:
    ...
    elif 'Attention' in args.agent_type and 'Listener' not in args.agent_type:
      from ReferentialGym.agents import AttentionLSTMCNNSpeaker
      speaker = AttentionLSTMCNNSpeaker(
        kwargs=agent_config, 
        obs_shape=obs_shape, 
        vocab_size=vocab_size, 
        max_sentence_length=max_sentence_length,
  ...
  elif 'AttentionListener' in args.agent_type and 'Speaker' not in args.agent_type:
      from ReferentialGym.agents import AttentionLSTMCNNListener
      listener = AttentionLSTMCNNListener(
        kwargs=listener_config, 
        obs_shape=obs_shape, 
        vocab_size=vocab_size, 
        max_sentence_length=max_sentence_length,
