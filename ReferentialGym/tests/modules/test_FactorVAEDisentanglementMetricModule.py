...
class ProgressiveShuffleModule(Module):
  ...
  def compute(self, input_streams_dict:Dict[str,object]) -> Dict[str,object]:
    ...
    if size != 0:
      indices_inp = torch.randint(low=0, high=inp.shape[0], size=(size,)).to(inp.device)
      indices_out = torch.randint(low=0, high=inp.shape[0], size=(size,)).to(inp.device)

      output[indices_out] = inp[indices_inp]
...
def main():
  ...
  save_path += "/MetricEPS1m5/TestFactorVAEDisentanglementMetricShuffleOHE/"
