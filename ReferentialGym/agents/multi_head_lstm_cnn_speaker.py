...
class MultiHeadLSTMCNNSpeaker(Speaker):
    def __init__(self, 
                    kwargs, 
                    multi_head_config,
                    obs_shape, 
        ...
        '''
        super(MultiHeadLSTMCNNSpeaker, self).__init__(obs_shape, 
            vocab_size, 
            max_sentence_length, 
            agent_id, 
            logger, 
        ...
        while len(self.multi_head_config['heads_archs']) != len(self.multi_head_config['heads_output_sizes']):
            self.multi_head_config['heads_archs'].append(copy.deepcopy(self.multi_head_config['heads_archs'][-1]))

        # Add output sizes to the archs:
        for idx, output_size in enumerate(self.multi_head_config['heads_output_sizes']):
            if isinstance(output_size, int):
                self.multi_head_config['heads_archs'][idx].append(output_size)
