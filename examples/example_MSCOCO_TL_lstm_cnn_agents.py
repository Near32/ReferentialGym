...
def main():
  ...
  # Transfer Learning:
  features_extractor = ReferentialGym.networks.ExtractorVGG16(input_shape=[3, 256, 256], final_layer_idx=final_layer_idx, pretrained=True)
  ...
  save_path = f"./Havrylov_et_al/TL/LSTMCNN/{rg_config['agent_loss_type']}/MSCOCO-OBS{rg_config['stimulus_resize_dim']}X{rg_config['stimulus_depth_dim']*rg_config['stimulus_depth_mult']}C"
  ...
  '''
  if 'CNN' == agent_config['architecture']:
    agent_config['cnn_encoder_channels'] = [64]
    agent_config['cnn_encoder_kernels'] = [3]
    agent_config['cnn_encoder_strides'] = [2]
    agent_config['cnn_encoder_paddings'] = [1,]
  ...
  train_dataset = ReferentialGym.datasets.MSCOCODataset(root=root, annFile=annFile, transform=transform, preloading=PRELOADING, extract_features=extract_features_fn)
  '''
  ...
  test_dataset = ReferentialGym.datasets.MSCOCODataset(root=root, 
                                                       ...
                                                       data_suffix=f'.VGG16-l{final_layer_idx}.npy')
